...
- **Duplicate `site_resource` artifacts** — **resolved**. The two capture paths now share a filename scheme: both Playwright's HAR attach and the agent's `NetworkCapture` write `bodies/<sha1>.<ext>`. Same content → same basename → host's basename-keyed merge in `Runner._collect_artifacts()` produces one artifact row per body. SHA-1 was chosen specifically to align with Playwright's native scheme; cryptographic strength is irrelevant for content-addressing in a controlled sandbox. The host's CAS still uses SHA-256 internally (computed at adoption time).

  **Why the duplication problem looked like an architectural flaw and isn't.** The instinct was "two capture systems for the same data → consolidate to one." Empirical evidence (`scripts/capture_diff.py` over a corpus of runs) showed each path is load-bearing:
  - HAR attach has a real race where Chromium disposes main-frame document bodies on fast redirects before Playwright reads them. The agent's `CDPResponseTap` closes the race by reading inside `loadingFinished`.
  - The agent's per-page CDP session does not reach cross-origin iframe targets (Chromium site isolation), so HAR is the only path that catches OOPIF documents.
  - The agent stack also has an observer-effect role: its own per-request CDP/route latency gives HAR's writer enough time to win the body-read race. Disabling either path will reintroduce gaps that two older runs in the corpus exhibited (`manifest_only > 0`) before the agent stack was complete.

  Future work that would still help: add a Sigma-style assertion in CI / nightly that re-runs `capture_diff.py` over recent runs and alerts if `manifest_only > N` or `HAR-only > M_real_bodies` — protects against silent regression of either path's coverage.
